...
Route::get('/test-variants', function () {
    return view('admin.products.test-variants');
})->middleware(['auth', IsAdmin::class]);

// Temporary route to fix super admin and check current user
Route::get('/fix-superadmin', function () {
    $user = \App\Models\User::where('email', 'superadmin@fashionshop.vn')->first();
    ...
    if (auth()->check()) {
        ...
        return response()->json([
            ...
            'user' => [
                ...
                'is_admin' => $user->is_admin,
                'is_admin_bool' => (bool)$user->is_admin,
                'is_admin_check' => $user->is_admin ? 'YES' : 'NO',
            ]
        ]);
    }
...
Route::get('/check-banners', function () {
    $banners = \App\Models\Banner::active()->forPage('home')->ordered()->get();
...
Route::get('/check-sections', function () {
    $sections = \App\Models\ProductSection::all();
...
Route::get('/test-upload/{bannerId}', function ($bannerId) {
    $banner = \App\Models\Banner::findOrFail($bannerId);
    return view('test-upload', compact('banner'));
});

Route::post('/test-upload/{bannerId}', function (\Illuminate\Http\Request $request, $bannerId) {
    $banner = \App\Models\Banner::findOrFail($bannerId);
    ...
    if ($request->hasFile('image')) {
        ...
        return redirect()->back()->with('success', 'Upload thành công! Image: ' . $path);
    }
    
    return redirect()->back()->with('error', 'Không có file được chọn');
});

// Test route for banner debugging
...
Route::get('/test-image-upload/{productId}', function ($productId) {
    return view('test-image-upload', compact('productId'));
})->middleware(['auth', IsAdmin::class]);
