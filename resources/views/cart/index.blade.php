...
<div class="container py-5">
    ...
    @if($carts->isEmpty())
        <div class="alert alert-info text-center py-5" role="alert">
            ...
            <a href="{{ route('products.index') }}" class="btn btn-primary mt-3"><i class="fas fa-shopping-bag"></i> Tiếp tục mua sắm</a>
        </div>
    @else
    <div class="row g-4">
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white border-bottom py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="mb-0"><i class="fas fa-list"></i> Danh sách sản phẩm ({{ $carts->count() }} mục)</h5>
                ...
                <div class="card-body p-0">
                    ...
                    <div class="border-bottom p-4 d-flex gap-3 align-items-start hover-shadow cart-item">
                        <div class="form-check mt-2">
                            <input class="form-check-input cart-checkbox" type="checkbox" value="{{ $item->id }}" 
                                   id="cart_{{ $item->id }}" onchange="updateTotal()">
                        </div>
                        ...
                        <div class="flex-grow-1">
                                ...
                                <div class="mb-2">
                                    @if($item->variant->color)
                                        <span class="text-muted small">
                                            <i class="fas fa-palette me-1"></i>Màu sắc: <strong>{{ ucfirst($item->variant->color) }}</strong>
                                        </span>
                                    @endif
                                    @if($item->variant->size)
                                        <span class="text-muted small ms-3">
                                            <i class="fas fa-ruler me-1"></i>Size: <strong>{{ $item->variant->size }}</strong>
                                        </span>
                                    @endif
                                </div>
                            @else
                            ...
                            <p class="text-muted small mb-2">
                                <i class="fas fa-tag me-1"></i>{{ $item->product->category->name ?? 'Chưa phân loại' }}
                            </p>
                            
                            @if($item->variant)
                                <p class="text-primary fw-bold mb-0" style="font-size: 1.1rem;">
                                    {{ number_format($item->variant->final_price, 0, ',', '.') }} ₫
                                </p>
                            @else
                            ...
                            <div class="d-flex align-items-center gap-2 mt-3">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div class="input-group" style="width: 130px;">
                                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="updateQuantity({{ $item->id }}, -1, {{ $item->variant ? $item->variant->stock_quantity : $item->product->quantity }})">
                                            <i class="fas fa-minus"></i>
                                        </button>
                                        <input type="number" id="qty_{{ $item->id }}" value="{{ $item->quantity }}" min="1" max="{{ $item->variant ? $item->variant->stock_quantity : $item->product->quantity }}" class="form-control form-control-sm text-center" style="width: 60px;" readonly />
                                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="updateQuantity({{ $item->id }}, 1, {{ $item->variant ? $item->variant->stock_quantity : $item->product->quantity }})">
                            ...
                            @if($maxStock < 5)
                                <p class="text-warning small mb-0 mt-2">
                                    <i class="fas fa-exclamation-triangle"></i> Chỉ còn {{ $maxStock }} sản phẩm
                                </p>
                            @endif
                        </div>
                        <div class="text-end text-nowrap">
                            ...
                            @endphp
                            <p class="fw-bold text-danger mb-0 item-total" style="font-size: 1.3rem;" data-price="{{ $itemTotal }}">
            ...
            <div class="card border-0 shadow-sm position-sticky" style="top: 20px;">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span id="displayTotal">{{ number_format($total,0,',','.') }} ₫</span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between fw-bold mb-2" style="font-size: 1.3rem;">
                        <span>Tổng cộng:</span>
                        <span class="text-danger" id="displayGrandTotal">{{ number_format($total,0,',','.') }} ₫</span>
                    </div>
                    <p class="small text-muted mb-4" id="selectedCount">Đã chọn {{ $carts->count() }} sản phẩm</p>
                    <form method="GET" action="{{ route('orders.create') }}" id="checkoutForm">
